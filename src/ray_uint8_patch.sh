...
python_folder_name=/home/ofir/anaconda3/envs/res_marl_3.7.7/lib/python3.7/

# Apply patches for https://github.com/ray-project/ray/issues/7946
sed -i -e '119s/tf.float32/tf.uint8/' "$python_folder_name"/site-packages/ray/rllib/policy/dynamic_tf_policy.py # Hardcoded observation space to uint8.
sed -i -e '76s/np.float32/np.uint8/' "$python_folder_name"/site-packages/ray/rllib/models/preprocessors.py # Same as above.
sed -i -e '231s/np.zeros(self.shape)/np.zeros(self.shape, dtype=self.observation_space.dtype)/' "$python_folder_name"/site-packages/ray/rllib/models/preprocessors.py # Change observation shape to what we actually provide
sed -i -e '214s/tf.int64/action_space.dtype/' "$python_folder_name"/site-packages/ray/rllib/models/catalog.py # Change action shape to what we actually provide
sed -i -e '56s/tf.math.argmax(self.inputs, axis=1)/tf.math.argmax(self.inputs, axis=1, output_type=tf.int32)/' "$python_folder_name"/site-packages/ray/rllib/models/tf/tf_action_dist.py # Actions should not sample at int64, int32 is the lowest that multinomial takes
sed -i -e '84s/tf.multinomial(self.inputs, 1)/tf.multinomial(self.inputs, 1, output_dtype=tf.int32)/' "$python_folder_name"/site-packages/ray/rllib/models/tf/tf_action_dist.py # Same as above
sed -i -e '656i\
        actions = np.array(actions, dtype=policy.action_space.dtype)
' "$python_folder_name"/site-packages/ray/rllib/evaluation/sampler.py # Insert action to uint8 conversion to save even more memory
...
' "$python_folder_name"/site-packages/ray/experimental/tf_utils.py
